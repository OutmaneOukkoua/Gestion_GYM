...
try {
    ...
    $stmt = Connexion::$cnx->query("select Id_Client,Prenom_Client,Nom_Client,Date_inscription,DateSup from archiveclient");
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);

    if (count($rows) > 0) {
        echo "<table>";
        echo "<tr><th>Id de Client</th><th>Prenom de Client</th>
        <th>Nom de Client</th><th>date inscription </th></th><th>date suppression </th>
        </td></tr>";
        foreach ($rows as $row) {
